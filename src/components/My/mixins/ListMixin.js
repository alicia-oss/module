 ...
 */
import {filterObj} from "@/components/_util/util";
...
export const ListMixin = {
  ...
  methods:{
    ...
    // 上一条下一条
    handleNext(){
      this.handleDetail(this.dataSource[this.currentModalIndex + 1],this.currentModalIndex+1);
    },
    handleLast(){
      this.handleDetail(this.dataSource[this.currentModalIndex - 1],this.currentModalIndex-1);
    },
    ...
    handlePreview(record,index){
      let ids =record.uploadFileName.split('?');
      if(ids.length <= 1){
        //直接预览
      }
      else{
